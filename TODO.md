...
- [ ] **pdf-vendor**: 390 KB → <300 KB
  ...
  - Исследовать: вынос jspdf в отдельный чанк
  - Исследовать: использование более лёгкой библиотеки для PDF
- [ ] **charts-core**: 529 KB → <400 KB
  - Текущий: 528.90 KB (145.79 KB gzipped)
  - Исследовать: lazy loading для отдельных графиков
  - Исследовать: tree-shaking для неиспользуемых компонентов Recharts

### 2. Ускорение сборки
- [ ] Сборка: 14.4s → <10s
  - Исследовать: SWC вместо TypeScript compiler
  - Исследовать: параллельная обработка TypeScript
  - Исследовать: кэширование зависимостей

### 3. Исправление предупреждений в тестах
- [ ] `useStatsWorker.test.ts` — warnings `act(...)` в 5 тестах
...
2. **Ускорение сборки** — 14.4s → <10s
3. **Исправление warnings в тестах** — act() warnings
4. **Supabase deployment** — если требуется backend функционал
